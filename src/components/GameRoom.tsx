...
async function fetchRoundAnswers(roundId: string): Promise<Answer[]> {
  const { data, error } = await supabase.from('answers').select('*').eq('round_id', roundId)
  if (error) throw error
  return (data as Answer[]) || []
}

/** Helper: get a not-yet-used question. Falls back to full bank only if all used. */
function pickUnusedQuestion(used: Set<string>): string {
...
async function refreshAll(
  ...
  try {
    const [g, p, r] = await Promise.all([fetchGame(gameId), fetchPlayers(gameId), fetchActiveRound(gameId)])
    setGame(g)
    setPlayers(p)
    setRound(r)
    if (r) {
      const a = await fetchRoundAnswers(r.id)
      setAnswers(a)
    } else {
      setAnswers([])
    }
...
export default function GameRoom({ gameId, playerId }: Props) {
  ...
  async function handleRefresh() {
    await refreshAll(gameId, setGame, setPlayers, setRound, setAnswers)
  }
  ...
  useEffect(() => {
    ...
    const ps = supabase
      ...
      .on(
        ...
        payload => {
          setPlayers(prev => prev.map(p => (p.id === (payload.new as any).id ? (payload.new as Player) : p)))
        }
      )
      .subscribe()

    const rs = supabase
      .channel('rounds_' + gameId)
      .on(
        'postgres_changes',
      ...
      .on(
        ...
        payload => {
          ...
          setRound(r)
          // when round changes, reload its answers
          fetchRoundAnswers(r.id).then(setAnswers).catch(() => {})
        }
      )
      .subscribe()
    ...
    return () => {
      supabase.removeChannel(gs)
      supabase.removeChannel(ps)
      supabase.removeChannel(rs)
      supabase.removeChannel(as)
    }
  }, [gameId])
  ...
  // Create first or next round with a not-yet-used question
  async function startOrNextRound() {
    ...
    const q = pickUnusedQuestion(used)
  ...
  // Skip now also ensures no-repeat within current game
  async function skipQuestion() {
    if (!round) return
    const { data: prev } = await supabase.from('rounds').select('question').eq('game_id', gameId)
    const used = new Set((prev || []).map((r: any) => r.question))
    if (round.question) used.add(round.question)
    const q = pickUnusedQuestion(used)
  ...
  async function submitMyAnswer(text: string) {
    ...
    if (mine) {
      ...
      await supabase
        .from('answers')
        .insert({ round_id: round.id, player_id: playerId, answer_text: text || '', done: true })
    }
  }
  ...
  async function submitScores() {
    ...
    const correctIds = answers
      .filter(a => a.round_id === round.id && a.is_correct === true)
      .map(a => a.player_id)
      ...
      <div style={{ display: 'flex', gap: 16 }}>
        <div className="card" style={{ flex: 1, padding: 12 }}>
          ...
          <ul className="list">
            {players.map(p => (
              <li key={p.id}>
                ...
                {isHost && !p.is_qm && (
                  <button className="btn btn-ghost" style={{ marginLeft: 8 }} onClick={() => assignQM(p)}>
                    Make QM
                  </button>
                )}
              </li>
          ...
          {isQM && (
            <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
              <button className="btn btn-danger" onClick={endGameNow}>
                End Game
              </button>
            </div>
          )}
        ...
        <div className="card" style={{ flex: 2, padding: 12 }}>
          {!round && (
            <div>
              {isQM ? (
                <button className="btn btn-primary" onClick={startOrNextRound}>
                  Start Round
                </button>
              ) : (
            ...
            <div>
              ...
              {round.status === 'reviewing' && isQM && (
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  <button className="btn btn-primary" onClick={approveQuestion}>
                    Approve
                  </button>
              ...
              {round.status === 'answering' && (
                <AnswerBox
                  ...
                  answers={answers.filter(a => a.round_id === round.id)}
                  players={players}
                />
              )}
                      ...
                      .map(a => {
                        ...
                        return (
                          <li key={a.id}>
                            ...
                            {a.revealed ? (a.answer_text || <em>(blank)</em>) : <em>hidden</em>}
                              ...
                              <span style={{ marginLeft: 8 }}>
                                <label>
                                  <input
                                    ...
                                    onChange={e => markCorrect(p.id, e.target.checked)}
                                  />{' '}
                                  correct
                                </label>
                              </span>
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        />
        <button className="btn btn-primary" onClick={() => onSubmit(text)} disabled={mine?.done === true}>
          {mine?.done ? 'Done' : 'Submit / Done'}
        </button>
      </div>
